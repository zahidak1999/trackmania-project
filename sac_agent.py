...
class LidarCNN(nn.Module):
    ...
    def __init__(self):
        ...
        self.conv1 = nn.Conv2d(4, 32, kernel_size=3, stride=2, padding=1)  # From (4, 64, 64) -> (32, 32, 32)
        self.conv2 = nn.Conv2d(32, 64, kernel_size=3, stride=2, padding=1) # From (32, 32, 32) -> (64, 16, 16)
        self.fc = nn.Linear(64 * 16 * 16, 64)  # Flatten the output and reduce to a vector of length 64
...
class SACAgent:
    ...
    def sample_action(self, obs):
        ...
        log_prob = dist.log_prob(x_t) - torch.log(1 - action.pow(2) + 1e-6)  # Use log_prob for the tanh trick
...
def preprocess_obs(obs, lidar_cnn):
    ...
    processed_obs = np.concatenate([speed, lidar, prev_actions, lidar_features.flatten()])
...
obs_dim = 1 + 1 + 3 + (8 * 8) # observation space using our CNN to transform our LIDAR images from (4, 64, 64) to (8 x 8)
...
for step in range(10000000):  # Total number of training steps
    ...
    act, log_prob = agent.sample_action(torch.tensor(processed_obs, dtype=torch.float32).unsqueeze(0).to(device))
    action = act.cpu().detach().numpy().flatten()  # Convert action to numpy for environment
    ...
    replay_buffer.store(processed_obs, action, reward, preprocess_obs(obs_next, lidar_cnn), done)
